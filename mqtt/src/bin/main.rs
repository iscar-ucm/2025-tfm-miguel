//! Aplicación MQTT sobre ESP32 (no_std + Embassy + rust-mqtt)
//!
//! Este programa realiza:
//! 1. Inicialización del sistema embebido (heap, timers, radio).
//! 2. Conexión a una red WiFi en modo cliente.
//! 3. Levantamiento de stack de red IPv4 con DHCP.
//! 4. Conexión TCP a un broker MQTT.
//! 5. Suscripción a un topic MQTT.
//! 6. Recepción de mensajes MQTT mediante polling.
...
async fn main(spawner: Spawner) {
    ...
    let mut client = Client::<'_, TcpSocket<'_>, _, 3, 3, 1, 16>::new(&mut buffer);
    ...
    let topic_target_yaw = TopicName::new(MqttString::from_str("/TFM/Target/Yaw").unwrap()).unwrap();
    ...
    {
        Ok(_) => {
            esp_println::println!("[MQTT] Suscripción al tpócio /TFM/Target/Yaw correcta");
        }
    ...
    // Bucle principal de recepción de mensajes MQTT
    loop {
        match client.poll().await {
            Ok(event) => {
                esp_println::println!("[MQTT] EVENTO: {:?}", event);

                if let Event::Publish(publish) = event {
                    let topic_str = publish.topic.as_ref().as_str();
                    esp_println::println!("[MQTT] TOPIC: {}", topic_str);
                    if let Ok(text) = core::str::from_utf8(&publish.message) {
                        esp_println::println!("[MQTT] MENSAJE: {}", text);
                    } else {
            ...
            Err(_) => {
                esp_println::println!("[MQTT] Error en la conexión MQTT...");
                break;
            }
        }
